...
public class MultiplayerState extends State {
	...
	private SButton HERO_SECOND;
	private SButton CONFIG;

	private int CODE1 = 0;
	private int CODE2 = 54;
	private int CODE3 = 55;

	private int SELECTED_FIRST_HERO;
	private int SELECTED_SECOND_HERO;

	public MultiplayerState(Program p) {
		super(p);
		...
		HERO_FIRST = new SButton(p, Constants.SPACE * 33, Constants.SPACE * 6, Constants.SPACE * 5, Constants.SPACE * 5, SELECTED_FIRST_HERO);
		HERO_SECOND = new SButton(p, Constants.SPACE * 33, Constants.SPACE * 14, Constants.SPACE * 5, Constants.SPACE * 5, SELECTED_SECOND_HERO);

		CONFIG = new SButton(p, Constants.SPACE * 33, Constants.SPACE * 20, Constants.SPACE * 5, Constants.SPACE * 2, "KEY", "BINDINGS", Constants.MENU_FONT_SMALLER);
	}
	...
	public void init() {
		...
		mapIndex = -1;
		displayIndex = 1;
		SELECTED_FIRST_HERO = CODE1;
		SELECTED_SECOND_HERO = CODE2;
	}
			...
			if (InputHandler.mouseClicked) {
				if (System.currentTimeMillis() - lastTime > 200) {
					...
					if (new Rectangle(PLAY.getX(), PLAY.getY(), PLAY.getWidth(), PLAY.getHeight()).contains(InputHandler.mouseX, InputHandler.mouseY)) {
						...
						if (SELECTED_FIRST_HERO == SELECTED_SECOND_HERO) {
							JOptionPane.showMessageDialog(null, "Same characters cannot be chosen!", "Warning", JOptionPane.WARNING_MESSAGE);
						} else {
							if (mapIndex != -1) {
								...
								for (int i = 0; i < selectedMapPackLevels.size(); i++) {
									...
									if (SELECTED_FIRST_HERO == CODE1 || SELECTED_SECOND_HERO == CODE1) {
										ninja = true;
									}
									if (SELECTED_FIRST_HERO == CODE2 || SELECTED_SECOND_HERO == CODE2) {
										robot = true;
									}
									if (SELECTED_FIRST_HERO == CODE3 || SELECTED_SECOND_HERO == CODE3) {
										knight = true;
									}
					...
					if (new Rectangle(HERO_FIRST.getX(), HERO_FIRST.getY(), HERO_FIRST.getWidth(), HERO_FIRST.getHeight()).contains(InputHandler.mouseX, InputHandler.mouseY)) {
						InputHandler.clearInput();
						if (SELECTED_FIRST_HERO == CODE1) {
							SELECTED_FIRST_HERO = CODE2;
						} else if (SELECTED_FIRST_HERO == CODE2) {
							SELECTED_FIRST_HERO = CODE3;
						} else if (SELECTED_FIRST_HERO == CODE3) {
							SELECTED_FIRST_HERO = CODE1;
						}
						HERO_FIRST.setType(SELECTED_FIRST_HERO);
					}
					if (new Rectangle(HERO_SECOND.getX(), HERO_SECOND.getY(), HERO_SECOND.getWidth(), HERO_SECOND.getHeight()).contains(InputHandler.mouseX, InputHandler.mouseY)) {
						InputHandler.clearInput();
						if (SELECTED_SECOND_HERO == CODE1) {
							SELECTED_SECOND_HERO = CODE2;
						} else if (SELECTED_SECOND_HERO == CODE2) {
							SELECTED_SECOND_HERO = CODE3;
						} else if (SELECTED_SECOND_HERO == CODE3) {
							SELECTED_SECOND_HERO = CODE1;
						}
						HERO_SECOND.setType(SELECTED_SECOND_HERO);
					}
					...
					if (new Rectangle(CONFIG.getX(), CONFIG.getY(), CONFIG.getWidth(), CONFIG.getHeight()).contains(InputHandler.mouseX, InputHandler.mouseY)) {
						InputHandler.clearInput();
						int dialogResult = JOptionPane.showConfirmDialog(null, "Old bindings will be lost!Are you sure you want to create new bindings?", "Warning", JOptionPane.YES_NO_OPTION);
	...
	public void draw(java.awt.Graphics g) {
		if (shouldDraw) {
			...
			g.drawString("   HERO 2", Constants.SPACE * 33, Constants.SPACE * 27 / 2);
