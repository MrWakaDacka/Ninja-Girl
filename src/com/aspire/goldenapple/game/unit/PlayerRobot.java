...
public class PlayerRobot extends Unit {
	...
	private static final long INVUL_DURATION = (int) Constants.TICK_PER_SEC * 3 / 2;
	...
	private ArrayList<RobotFireBall> fireBalls;
	...
	public PlayerRobot(Program program, GamePanel gamePanel, int x, int y, int width, int height, int status, int priority) {
		...
		score = 0;
		shouldWork = true;
		fireBalls = new ArrayList<RobotFireBall>();
	...
	public void update() {
		if (shouldUpdate) {
			...
			if (alive) {
				...
				if (focus) {
					focus = false;
					if (gamePanel.robotPlaying && !gamePanel.robotFocus) {
						gamePanel.ninjaFocus = false;
						gamePanel.knightFocus = false;
						gamePanel.robotFocus = true;
					}
				}

				if (fire) {
					if (ticks - kunaiTicks >= FIRE_COOLDOWN) {
						kunaiTicks = ticks;
						fire();
					}
				}
			...
			Iterator<RobotFireBall> iter = fireBalls.iterator();
			while (iter.hasNext()) {
				RobotFireBall k = iter.next();
				if (!k.shouldWork) {
					iter.remove();
				} else {
					k.update();
	...
	public void fire() {
		...
		fireBalls.add(new RobotFireBall(program, gamePanel, x, y, Constants.BRICK_SIZE / 3 * 2, Constants.BRICK_SIZE / 3 * 2, direction, status, priority));
