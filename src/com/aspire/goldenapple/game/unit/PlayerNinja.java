...
public class PlayerNinja extends Unit {
	...
	private static final int MAX_JUMP_HEIGHT = Constants.BRICK_SIZE * 5;
	...
	public PlayerNinja(Program program, GamePanel gamePanel, int x, int y, int width, int height, int status, int priority) {
	...
	public void update() {
		if (shouldUpdate) {
			...
			if (alive) {
				...
				if (focus) {
					focus = false;
					if (gamePanel.ninjaPlaying && !gamePanel.ninjaFocus) {
						gamePanel.ninjaFocus = true;
